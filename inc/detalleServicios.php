<?php 
if (!isset($_SESSION['sesionActiva'])||isset($_SESSION['sesionActiva'])){
  ...
  if ($total >= 1) {
    ...
    do{      
      ...
      if (isset($_SESSION['sesionActiva']) && $_SESSION['tipoSesion']=="1") {
        ...
        $modificar = '<a class="btn-xs btn-default btn-group-sm-6" href="management/clientes/gestionar-servicio.php?id='.$datos['srvc_id'].'"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>Editar</a>';
        $eliminar = '<a class="btn-xs btn-danger btn-group-sm-6" href="management/clientes/gestionar-servicio.php?delete&id='.$datos['srvc_id'].'"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span>Eliminar</a>';
        }
      }
      ...
      $datos['ja_tipousuario']=="1"){

        $salida = $salida.'<br><small>(Publicado por: '.$datos['NombreManager'].')</small>';
      }elseif(isset($_SESSION['sesionActiva'])&&$_SESSION['tipoSesion']==$datos['pk_srvc_tipoUsuario']&&$_SESSION['idSesion']==$datos['pk_srvc_idSesion']&&$datos['cli_tipousuario']=="10"){
        $salida = $salida.'<br><small>(Publicado por: '.$datos['NombreCliente'].')</small>';
      }*/
      $salida = $salida . '</div>';
  ...
  echo '<h5><a href="registro.php" class="btn-xs btn-success">Registrate</a> para ver los servicios de nuestro sitio o <a href="login.php" class="btn btn-info">Inicia Sesi&oacute;n</a></h5>';
}
?>